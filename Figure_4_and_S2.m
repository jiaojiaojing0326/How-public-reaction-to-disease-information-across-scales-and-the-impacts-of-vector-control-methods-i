...
% Matlab code to generate Figure 4 and S2 in Appendix for manuscript titled "How public reaction to disease information across scales and the impacts of vector control methods influence disease prevalence and control efficacy"

% =========================================================================================================================================================================================================================================
 
% This code can generate all csv. files used in R code "Figure_4.R": by
% modifying the KL for three levels of breeding capacities, modifying
% information to fit all three levels of disease information. 

function [S_out, I_out, R_out]  = Multiple_Patch_Time_and_Strength(folder, information, MaxTime, p, alpha, gamma, epsilon, N, seed, network)
...
cont = 0:20:200; % result at different levels of adult concerns

for h = 1: max(size(cont))
h

if nargin == 0                          % if the number of input arguments is zero
    seed     = randi(intmax('int32'),1,1);
    MaxTime  = 1000;                     % end of simulation
    epsilon  = [(cont(h)+0.0001) (200.0001-cont(h))] ;              % environment concern demotivates [mosquitos  larvae] pesticide
    gamma    = exp(-epsilon/80);        % motivation to control [mosquito adult,  larvae] due to Infected humans
    alpha    = 100 * gamma;             % motivation to control [mosquito adult,  larvae] due to Severe Outcomes (always 100 times larger than the regular infected)
    ...
    information = 'global';             % 'global', 'neighbor'or 'local' disease information
    folder = join([network, information, 'N', string(N),'p', string(p), 't', string(MaxTime),'K=500','time+strength+control'], {'-'} ); %folder to save all results
    mkdir(folder)
end
...
for q = 1:50 %q as repeat time to make the results more stable
    
%initial conditions in each patch
X0 = [];
Kl = []; % there are three levels of breeding capacity KL. Here we assume KL=500 in the following for loop, 800 and 2000 can be achieved by changing that value in KL vector.
%delay = [];
for i = 1:N
    ...
    X0  = [X0; ini];
    Kl  = [Kl; 500 + randi(10,1,1)];  %breeding capacity of each patch: 20000 + randi(5000,1,1)
end
...
% save all the variables as function of time in a text file for plotting later 
% timeaxis = pop.x.';
...
function dX = SIR_eq_local(t, X)
    ...
    for j = 0:N-1 
        ...
        tmp = alpha(1) * (X(4+m*j)) + gamma(1) * (X(2+m*j)) - (epsilon(1) * X(8+m*j)); % mosquito control
        dX(8+j*m) = check_neg_or_one(X(8+m*j), tmp);
        dX(8+j*m) = check_neg(X(8+m*j), tmp);
        
        tmp = (alpha(2) * (X(4+m*j)) + gamma(2) * (X(2+m*j)) - (epsilon(2) * X(9+m*j)))*(1- X(8 + j*m)/(0.00001 + X(8 + j*m) + X(9 + j*m))); % larvae control
...
function dX = SIR_eq_neighbor(t, X)
    ...
    for j = 0:N-1 
        ...
        tmp = alpha(1) * (Dh_neighbor + X(4+m*j))+ gamma(1) * (Ih_neighbor + X(2+m*j)) - (epsilon(1) * X(8+m*j)); % mosquito control
        dX(8+j*m) = check_neg_or_one(X(8+m*j), tmp);
        dX(8+j*m) = check_neg(X(8+m*j), tmp);
        
        tmp = (alpha(2) * (Dh_neighbor + X(4+m*j)) + gamma(2) * (Ih_neighbor + X(2+m*j)) - (epsilon(2) * X(9+m*j)))*(1- X(8 + j*m)/(0.00001 + X(8 + j*m) + X(9 + j*m))); % larvae control
        dX(9+j*m) = check_neg_or_one(X(9+m*j), tmp);
        dX(9+j*m) = check_neg(X(9+m*j), tmp);
...
function dX = SIR_eq_global(t, X)
    ...
    for j = 0:N-1 
        ...
        tmp = alpha(1) * Dh_tot + gamma(1) * Ih_tot - (epsilon(1) * X(8+m*j)); % mosquito control
        dX(8+j*m) = check_neg_or_one(X(8+m*j), tmp);
        dX(8+j*m) = check_neg(X(8+m*j), tmp);
        
        tmp = (alpha(2) * Dh_tot + gamma(2) * Ih_tot - (epsilon(2) * X(9+m*j)))*(1- X(8 + j*m)/(0.00001 + X(8 + j*m) + X(9 + j*m))); % larvae control
        dX(9+j*m) = check_neg_or_one(X(9+m*j), tmp);
        dX(9+j*m) = check_neg(X(9+m*j), tmp);
