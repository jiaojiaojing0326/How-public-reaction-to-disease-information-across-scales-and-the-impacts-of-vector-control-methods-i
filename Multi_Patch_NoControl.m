...
% Matlab code to generate Figure 4 for manuscript titled "How public reaction to disease information across scales and the impacts of vector control methods influence disease prevalence and control efficacy"

% =========================================================================================================================================================================================================================================
 
% This code generates all results in the absence of disease control such as
% calculating the control efficacy in Fig. 3 and 4. By
% modifying the KL for three levels of breeding capacities, modifying
% information to fit all three levels of disease information, we can get all No Control results in the combination of disease information and breeding capacities. 

function [S_out, I_out, R_out]  = Multi_Patch_NoControl(folder, information, MaxTime, p, alpha, gamma, epsilon, N, seed, network)
...
if nargin == 0                          % if the number of input arguments is zero
    ...
    folder = join([network, information, 'N', string(N),'p', string(p), 't', string(MaxTime),'K=2000','nocontrol'], {'-'} );
    mkdir(folder)
end
...
for i = 1:N
    ...
    X0  = [X0; ini];
    Kl  = [Kl; 2000 + randi(10,1,1)];  %carrying capacity of each patch: 2000 + randi(5000,1,1), can be modified at 500 and 800 to generate different results
end
...
function dX = SIR_eq_local(t, X)
    ...
    for j = 0:N-1 
        ...
        tmp = 0*(alpha(1) * (X(4+m*j)) + gamma(1) * (X(2+m*j)) - (epsilon(1) * X(8+m*j))); % mosquito control = 0
        dX(8+j*m) = check_neg_or_one(X(8+m*j), tmp);
        dX(8+j*m) = check_neg(X(8+m*j), tmp);
        
        tmp = 0*((alpha(2) * (X(4+m*j)) + gamma(2) * (X(2+m*j)) - (epsilon(2) * X(9+m*j)))*(1- X(8 + j*m)/(0.00001 + X(8 + j*m) + X(9 + j*m)))); % larvae control
...
function dX = SIR_eq_neighbor(t, X)
    ...
    for j = 0:N-1 
        ...
        tmp = 0*(alpha(1) * (Dh_neighbor + X(4+m*j))* binary(X(4+m*j))+ gamma(1) * (Ih_neighbor + X(2+m*j))* binary(X(2+m*j)) - (epsilon(1) * X(8+m*j))); % mosquito control
        dX(8+j*m) = check_neg_or_one(X(8+m*j), tmp);
        dX(8+j*m) = check_neg(X(8+m*j), tmp);
        ...
        tmp = 0*(alpha(2) * (Dh_neighbor + X(4+m*j))* binary(X(4+m*j)) + gamma(2) * (Ih_neighbor + X(2+m*j))* binary(X(2+m*j)) - (epsilon(2) * X(9+m*j))); % larvae control
        dX(9+j*m) = check_neg_or_one(X(9+m*j), tmp);
        dX(9+j*m) = check_neg(X(9+m*j), tmp);
...
function dX = SIR_eq_global(t, X)
    ...
    for j = 0:N-1 
        ...
        tmp = 0*( alpha(1) * Dh_tot * binary(X(4+m*j)) + gamma(1) * Ih_tot * binary(X(2+m*j))- (epsilon(1) * X(8+m*j))) ; % mosquito control
        dX(8+j*m) = check_neg_or_one(X(8+m*j), tmp);
        dX(8+j*m) = check_neg(X(8+m*j), tmp);
        
        %tmp = (alpha(2) * Dh_tot + gamma(2) * Ih_tot - (epsilon(2) * X(9+m*j)))*(1- X(8 + j*m)/(0.00001 + X(8 + j*m) + X(9 + j*m))); % larvae control
        tmp = 0*(alpha(2) * Dh_tot * binary(X(4+m*j)) + gamma(2) * Ih_tot * binary(X(2+m*j))- (epsilon(2) * X(9+m*j))); % larvae control
        dX(9+j*m) = check_neg_or_one(X(9+m*j), tmp);
        dX(9+j*m) = check_neg(X(9+m*j), tmp);
